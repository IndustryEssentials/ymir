const tip = {
  ...
  "tip.task.fusion.includelable": { cn: "期望生成的数据集包含选中的标签值", en: "Filtering tips: the generated dataset  expect to contain the selected keyword", },
  "tip.task.fusion.excludelable": { cn: "期望生成的数据集不包含选中的标签值", en: "Excluding tips: the generated dataset expect to not contain the selected keyword", },
  "tip.task.fusion.sampling": { cn: "期望的最大采样数量", en: "Samples", },
  ...
  "tip.task.train.image": { cn: "平台提供默认训练镜像，同时支持开发者自主开发并接入。镜像会附带默认配置参数，可按需调整。", 
  ...
  "tip.task.mining.image": { cn: "平台提供默认挖掘镜像，同时支持开发者自主开发并接入。镜像会附带默认配置参数，可按需调整。", 
  ...
  "tip.task.inference.image": { cn: "平台提供默认推理镜像，同时支持开发者自主开发并接入。镜像会附带默认配置参数，可按需调整。", 
  ...
  "tip.task.filter.imodel": { cn: "推理出来的数据一般用于该模型的效果优化", en: "The inferential dataset is generally used for the optimization of the selected model", },
  "tip.task.filter.igpucount": { cn: "所选GPU个数越多，推理速度越快，请注意资源合理分配", en: "The more GPUs you select, the faster the inferential speed will be, please allocate resources reasonably", },
  "tip.task.filter.ihyperparams": { cn: "推理镜像中需传入的运行参数，默认为最佳推荐配置", en: "The operation parameters to be entered in the inferential docker, the default value is the best recommended configuration", },
