...
const DatasetMoreInfo: FC<{ task: YModels.Task }> = ({ task }) => {
  ...
  function renderDatasetName(id: number) {
  ...
  function renderModel(id: number, pid: number, label = 'task.mining.form.model.label') {
    return id ? (
      <Item label={t(label)}>
  ...
  function renderDuration(label: string) {
    return label ? <Item label={t('task.column.duration')}>{label}</Item> : null
  }

  function renderKeepAnnotations(type: number) {
  ...
  function renderImage(imageName: ReactNode, span = 1, label = 'task.detail.label.training.image') {
    return (
  ...
  function renderImageById(id: number, span: number, label: string) {
    return renderImage(<ImageName id={id} />, span, label)
  }

  function renderImageByUrl(url: string, span: number, label: string) {
    return url ? renderImage(<ImageName url={url} />, span, label) : null
  }

  function renderDatasetSource(id: number) {
  ...
  function renderCreateTime(time: string) {
...
export default DatasetMoreInfo
