...
const ProjectEmpty: FC<{ style?: CSSProperties; addExample?: () => void }> = ({ style, addExample = () => {} }) => {
  ...
  return (
    <Space className={s.empty} style={style} direction="vertical">
      ...
      <Space className={s.actions}>
        <Button className={s.addBtn} type="primary" onClick={() => history.push('/home/project/add')} icon={<AddIcon />}>
          {t('project.new.label')}
        </Button>
        <Button className={s.addBtn} type="primary" onClick={() => addExample()} icon={<AddIcon />}>
