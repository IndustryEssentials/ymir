...
-   标注/GroundTruth(GT)：数据集中正确的标注值，作为待识别目标的参考标注，一般情况下为人工标注。
    
-   推理结果/Prediction：数据集经过模型推理后产生的标注结果，用于评估模型的识别效果。
...
FP：False Positive，当前测试图片的标注不包含正确的检测目标，但模型将其错误地识别为了检测目标。即预测结果中被预测为正类的负样本。

FN：False Negative，当前测试图片的标注为正确的检测目标，但模型未识别到或将其错误地识别为了其他目标。即预测结果中被预测为负类的正样本。

TP：True Positive，即在目标预测类别下，和标注匹配的模型预测结果。
