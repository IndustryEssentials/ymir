...
def _process_results(mir_root: str, label_storage_file: str, export_out: str, src_typ_rev_tid: revs_parser.TypRevTid,
    ...
    for asset_id in set(mir_annotations.ground_truth.image_annotations.keys()) & asset_ids_set:
        matched_mir_annotations.ground_truth.image_annotations[asset_id].CopyFrom(
            mir_annotations.ground_truth.image_annotations[asset_id])
    for asset_id in set(mir_annotations.prediction.image_annotations.keys()) & asset_ids_set:
        matched_mir_annotations.prediction.image_annotations[asset_id].CopyFrom(
    ...
    if add_prediction:
        matched_prediction = matched_mir_annotations.prediction
        matched_prediction.Clear()
        ...
        file_names = list(infer_result_prediction.image_annotations.keys())
        for file_name in file_names:
            asset_id = os.path.splitext(file_name)[0]
            if asset_id in asset_ids_set:
                matched_prediction.image_annotations[asset_id].CopyFrom(
                    infer_result_prediction.image_annotations[file_name])

        # pred type and meta
        matched_prediction.eval_class_ids[:] = set(
        ...
        matched_prediction.type = model_storage.object_type  # type: ignore
