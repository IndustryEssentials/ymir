...
class Asset(BaseModel):
    ...
    def format_assets_info(
        cls, assets_content: Dict, offset: int, limit: int, class_id: int
    ) -> Dict:
        """
        ...
        """
        asset_ids = assets_content["class_ids_index"][class_id]["asset_ids"][
            offset : limit + offset
        ]
        elements = [
            dict(
    ...
    @classmethod
    def get_assets_info_from_cache(
        cls, cache_key: str, offset: int, limit: int, class_id: int
    ) -> Dict:
        """
        return structure like this:
        {'class_ids_count': {3: 34}, 'elements': [{'asset_id':xxx, 'class_ids':[2,3]},],
        ...
        assets_detail = redis_cache.hmget(
            f"{cache_key}:{config.ASSET_ID_DETAIL}", asset_ids
        )

        elements = []
        for asset_id, asset_detail in zip(asset_ids, assets_detail):
            elements.append(
                dict(asset_id=asset_id, class_ids=json.loads(asset_detail)["class_ids"])
            )
        assets_attributes = redis_cache.get(f"{cache_key}:{config.ASSETS_ATTRIBUTES}")
        total = redis_cache.llen(
            f"{cache_key}:{config.ASSETS_CLASS_ID_INDEX}:{class_id}"
        )
    ...
    def get_assets_info(self, offset: int, limit: int, class_id: int) -> Dict:
        ...
        if self.check_cache_existence():
            result = self.get_assets_info_from_cache(
                self.cache_key, offset, limit, class_id
            )
    ...
    @classmethod
    def get_asset_id_info_from_cache(
        cls, cache_key: str, asset_id: str
    ) -> Union[Dict, None]:
        """
        ...
        """
        asset_id_info = redis_cache.hget(
            f"{cache_key}:{config.ASSET_ID_DETAIL}", asset_id
        )
