...
from update_1_1_0_to_1_3_0.step_updater import update_all as update_110_130, update_user_labels as update_labels_110_130


_RepoUpdaterType = Callable[[str], None]
_LabelsUpdaterType = Callable[[str], None]
...
def _get_update_steps(src_ver: str, dst_ver: str) -> List[Tuple[_RepoUpdaterType, _LabelsUpdaterType]]:
    eq_src_ver = ymir_salient_version(src_ver)
    eq_dst_ver = ymir_salient_version(dst_ver)

    _UPDATE_NODES: List[str] = ['1.1.0', '1.3.0']
    _UPDATE_FUNCS: List[Tuple[_RepoUpdaterType, _LabelsUpdaterType]] = [(update_110_130, update_labels_110_130)]
