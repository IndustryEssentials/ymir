...
class TaskImportDatasetInvoker(TaskBaseInvoker):
    ...
    def subtask_invoke_import(cls, request: backend_pb2.GeneralReq, user_labels: UserLabels, sandbox_root: str,
        ...
        import_dataset_response = cls.importing_cmd(
            ...
            anno_type=import_dataset_request.anno_type)
    ...
    @staticmethod
    def importing_cmd(repo_root: str, label_storage_file: str, task_id: str, index_file: str, pred_path: str,
                      gt_path: str, media_location: str, work_dir: str,
        ...
        importing_cmd = [
            ...
            '-w', work_dir, "--user-label-file", label_storage_file, "--anno-type", utils.anno_type_str(anno_type)
        ]
        if pred_path:
            importing_cmd.extend(['--pred-path', pred_path])
        if gt_path:
            importing_cmd.extend(['--gt-path', gt_path])
