...
class TestInvokerTaskExporting(unittest.TestCase):
    ...
    def test_invoker_00(self, mock_run):
        exporting_request = mirsvrpb.TaskReqExporting()
        exporting_request.dataset_id = self._base_task_id
        exporting_request.format = mir_common.LabelFormat.PASCAL_VOC
        ...
        req_create_task.task_type = mir_common.TaskTypeExportData
        ...
        working_dir = os.path.join(self._sandbox_root, "work_dir",
                                   mir_common.TaskType.Name(mir_common.TaskTypeExportData), self._task_id)
