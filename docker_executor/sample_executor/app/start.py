...
def _run_training(env_config: env.EnvConfig) -> None:
    ...
    #! use `monitor.write_training_result` to save training result
    monitor.write_training_result(model_names=['model-0000.params', 'model-symbols.json'],
                                  mAP=0.8,
...
def _run_infer(env_config: env.EnvConfig) -> None:
    ...
    #! write infer result
    fake_annotation = monitor.Annotation(class_name='cat', score=0.9, box=monitor.Box(x=50, y=50, w=150, h=150))
