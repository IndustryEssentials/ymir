...
YMIR(You Mine In Recursive)是一个简化的模型开发产品，专注于AI SUITE开源系列中的数据集版本和模型迭代。
...
YMIR平台主要满足用户规模化生产模型的需求，为用户提供良好、易用的展示界面，便于数据和模型的管理与查看。平台包含主页、任务管理、数据集管理、模型管理四个功能模块，支持实现以下主要功能：

| 功能模块     |    一级功能    | 二级功能          | 功能说明      |
|----------|-----------|------------|-----------------------------------------|
| 数据管理     |     数据集管理      |     导入数据集       | 支持用户通过复制公共数据集、url地址、路径导入以及本地导入等方式导入准备好的数据集|
|         |     数据集管理      |     查看数据集       | 支持图片数据及标注的可视化查看、以及历史信息的查看|
|         |     数据集管理      |     编辑数据集       | 支持数据集的编辑、删除|
|         |     任务管理      |     筛选任务       | 支持根据标签对数据集进行筛选|
| 训练模型     |    任务管理       |     训练任务       |     支持自选数据集、标签，并根据需求调整训练参数来训练模型，完成后可查看对应的模型效果                          |
| 模型验证     |    模型管理       |      模型验证      |    支持上传单张图片，通过可视化的方式查看模型在真实图片中的表现，以校验模型的精确度                                           |
| 数据挖掘        | 任务管理   |       挖掘任务      | 支持在海量数据集中找到对模型优化最有利的数据                                                |
| 数据标注        | 任务管理      |      标注任务       | 支持为图片数据添加标注                                                 |
...
因此，YMIR平台通过主动学习的方法，首先通过少量数据来训练出一个初始模型，使用该初始模型，从海量数据中挖掘出对模型能力提高最有利的数据。挖掘完成后，仅针对这部分数据进行标注，对原本的训练数据集进行高效扩充。
...
输入任务名称（必填），选择训练集（VOC2012_train），选择测试集（VOC2012_val），选择训练目标（person，car），选择训练类型、算法框架、骨干网络结构、GPU个数以及配置训练参数（训练参数提供默认值，默认参数中key值不可修改，value值可修改，如需添加参数可以自行添加），点击创建任务。如下图所示，训练初始模型：

![训练2](docs/images/%E8%AE%AD%E7%BB%832.jpeg)

创建成功后，跳转到任务管理界面，可以查看到相应的任务进度和信息，任务完成后可查看模型的效果（mAP值）。
...
1. 后端 [https://github.com/IndustryEssentials/ymir-backend](https://github.com/IndustryEssentials/ymir-backend)，负责任务分发与管理

2. 前端 [https://github.com/IndustryEssentials/ymir-web](https://github.com/IndustryEssentials/ymir-web)，交互界面

3. 命令行 [https://github.com/IndustryEssentials/ymir-cmd](https://github.com/IndustryEssentials/ymir-cmd)，CLI界面，管理底层标注与图像数据
