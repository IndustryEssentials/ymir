...
在此场景中，用户想在某个数据集上，或者某些图片上，对模型的效果进行初步验证，用户通过 `mir infer` 命令，或一个带有 `add_annotations` 参数的 `mir mining` 命令启动这个过程，这个过程分为以下几个步骤：
...
* `run_infer`: 取值为0或1，1表示需要将检测/分类结果写入 `infer-result.json` 中，0表示不需要进行推理
