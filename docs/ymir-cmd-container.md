...
3. 训练成功完成后，ymir-cmd 读取位于 `out/models` 目录下的输出模型，校验其有效性，并将模型归档。
...
3. 推理成功完成后，ymir-cmd 读取位于 `out/infer-result.json` 位置下的输出结果，如果是对于某个特定数据集（而不是图片）进行的推理，则将推理结果写入数据集中，形成一个新的数据集。
...
4. 镜像完成后，ymir-cmd 收集 `/out/result.tsv` 中的结果，根据其中 topk 的结果筛选数据集，并将
